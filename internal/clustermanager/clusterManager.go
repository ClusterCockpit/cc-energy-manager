...
func (cm *clusterManager) StartJob(startJobData schema.Job) {
	...
	if !subCluster.partitionRegex.MatchString(startJobData.Partition) {
		cclog.Debugf("Ignoring job (%s, %s, %d), which isn't a whitelisted partition (%s)", cluster, subCluster, startJobData.JobID, startJobData.Partition)
		return
	}
...
func (cm *clusterManager) UpdateJobPowerLimits(cluster string) error {
	...
	for jobId, job := range cm.clusters[cluster].jobIdToJob {
		for deviceId, jobManager := range job.deviceTypeToJobMgr {
			...
			cclog.Debugf("Setting power budget for job %v to %v", jobId, power)
